...
max_pct_id = float(sys.argv[5])
min_clust_size = int(sys.argv[6])
num_seq_pairs = int(sys.argv[7])
out_1 = 'A_' + sys.argv[8]
out_2 = 'B_' + sys.argv[8]
...
def read_cluster(cluster_file, min_length, min_pct_id, max_pct_id):
    sequences = []
        ...
        for line in file:
            ...
            if line[0] == '>':
                if min_length <= len(seq) <= max_seq_len:
                    ...
                    if max_pct_id >= newSequence.pct_id >= min_pct_id and newSequence.t_end - newSequence.t_start > (min_length / 2.0):
...
def read_all_clusters(clusters, min_clust_size, min_length, min_pct_id, max_pct_id):
    cluster_sequences = []
    num_sequences = 0
    for c in clusters:
        new_sequences = read_cluster(c.file_path, min_length, min_pct_id, max_pct_id)
...
clusters, total_seqs = read_all_clusters(clusters, min_clust_size, min_seq_len, min_pct_id, max_pct_id)
...
for i in range(num_seq_pairs):
    ...
    tseq2 = 0
    while tseq1 == tseq2:
        tseq1 = random.randint(0, len(c) - 1)
        tseq2 = random.randint(0, len(c) - 1)
